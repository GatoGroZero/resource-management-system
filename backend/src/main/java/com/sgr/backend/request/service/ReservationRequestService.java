...
public class ReservationRequestService {
    ...
    public ReservationRequest approve(Long id, RequestDecisionRequest request) {
        ReservationRequest reservationRequest = reservationRequestRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Solicitud no encontrada"));

        User admin = userRepository.findByEmail(request.getAdminEmail())
                .orElseThrow(() -> new RuntimeException("Admin no encontrado"));

        reservationRequest.setStatus(RequestStatus.APPROVED);
        reservationRequest.setResponseMessage(request.getResponseMessage());
        reservationRequest.setReviewedBy(admin);
    ...
    public ReservationRequest reject(Long id, RequestDecisionRequest request) {
        ...
        reservationRequest.setStatus(RequestStatus.REJECTED);
        reservationRequest.setResponseMessage(request.getResponseMessage());
        reservationRequest.setReviewedBy(admin);
        reservationRequest.setReviewedAt(LocalDateTime.now());
